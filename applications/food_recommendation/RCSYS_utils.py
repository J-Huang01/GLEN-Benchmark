...
def read_graph_data(path='./graph/hetero_graph_bipartite.pt'):
...
def diversity_loss(users_emb_final, pos_items_emb_final, neg_items_emb_final, user_features_batch, pos_item_features_batch, neg_item_features_batch, k=20):
    ...
    def get_mean_similarity(user_features_batch, item_features_batch, k):
        ...
        # Get the food features for the top K items for each user
        # top_k_items_features shape: (batch_size, k, feature_dim)
        top_k_items_features = item_features_batch[top_k_indices]
        
        # Calculate pairwise similarities within the top K items for each user
        # We need to compare each pair of items
        batch_size, top_k, feature_dim = top_k_items_features.shape
        
        # Reshape to (batch_size * k, feature_dim)
        flat_features = top_k_items_features.view(-1, feature_dim)
        
        # Calculate cosine similarity matrix for all pairs in the batch
        # This is expensive but necessary for diversity calculation
        # cosine_sim shape: (batch_size * k, batch_size * k)
        flat_features_norm = flat_features / (flat_features.norm(dim=1, keepdim=True) + 1e-8)
        cosine_sim = torch.mm(flat_features_norm, flat_features_norm.t())
        
        # Reshape to (batch_size, k, batch_size, k)
        cosine_sim = cosine_sim.view(batch_size, top_k, batch_size, top_k)
        
        # We only want similarities within the same batch (same user)
        # Extract diagonal blocks
        user_similarities = []
        for i in range(batch_size):
            user_sim = cosine_sim[i, :, i, :]  # shape: (k, k)
            # Get upper triangular part (excluding diagonal) to avoid counting same item similarity
            mask = torch.triu(torch.ones_like(user_sim), diagonal=1).bool()
            user_sim_values = user_sim[mask]
            if user_sim_values.numel() > 0:
                user_similarities.append(user_sim_values.mean())
            else:
                user_similarities.append(torch.tensor(0.0, device=user_sim.device))
        
        mean_similarity = torch.stack(user_similarities).mean()
        return mean_similarity
    
    # Calculate mean similarity (we want to minimize this for diversity)
    mean_sim = get_mean_similarity(user_features_batch, pos_item_features_batch, k)
    
    # Diversity loss: we want high diversity (low similarity)
    # So we return the mean similarity as the loss (minimizing similarity = maximizing diversity)
...
def get_user_positive_items(edge_index):
    """Builds dictionary of positive items for each user
...
def RecallPrecision_ATk(groundTruth, r, k):
    ...
    """
    device = r.device if hasattr(r, 'device') else torch.device('cpu')
...
def NDCGatK_r(groundTruth, r, k):
    ...
    test_matrix = torch.zeros((len(r), k), device=device)

    for i, items in enumerate(groundTruth):
        length = min(len(items), k)
        test_matrix[i, :length] = 1
    max_r = test_matrix
    idcg = torch.sum(max_r * 1. / torch.log2(torch.arange(2, k + 2, device=device)), axis=1)
    dcg = r * (1. / torch.log2(torch.arange(2, k + 2, device=device)))
...
def calculate_health_score(users, top_K_items, user_tag, food_tag, num_nutrition_tags=18):
    """
        
    Args:
        users: user indices
        top_K_items: top K recommended items for each user
        user_tag: all user tags
        food_tag: all food tags
        num_nutrition_tags: number of nutrition-related tags (default 18, indices 0-17)
    
    Returns:
        float: health score (average proportion of healthy foods per user)
    """
    # Ensure tensors are on the CPU
    user_tags = user_tag[users].cpu()  # Shape: (num_users, total_tags)
    recommended_items = top_K_items[users].cpu()  # Shape: (num_users, k)
    
    # Get the food tags for the top K items for all users
    food_tags = food_tag[recommended_items].cpu()  # Shape: (num_users, k, total_tags)

    user_nutrition = user_tags[:, :num_nutrition_tags]  # Shape: (num_users, num_nutrition_tags)
    food_nutrition = food_tags[:, :, :num_nutrition_tags]  # Shape: (num_users, k, num_nutrition_tags)
    
    # Expand dimensions for broadcasting
    user_nutrition_expanded = user_nutrition.unsqueeze(1)  # Shape: (num_users, 1, num_nutrition_tags)
    
    # Calculate if there's at least one common nutrition tag (intersection > 0)
    common_tag = torch.logical_and(user_nutrition_expanded, food_nutrition).sum(dim=2) > 0  # Shape: (num_users, k)
...
def calculate_poverty_affordability(users, top_K_items, user_tag, food_tag, num_poverty_tags=3):
    """    
    Calculates the poverty affordability score based on the presence of at least one common 
    poverty tag between user and food tags.
    ...
    Args:
        ...
        user_tag: all user tags
        food_tag: all food tags
        num_poverty_tags: number of poverty-related tags (default 3, last 3 indices)
    
    Returns:
        float: poverty affordability score (average proportion of affordable foods per user)
    """
    # Ensure tensors are on the CPU
    ...
    user_poverty = user_tags[:, -num_poverty_tags:]  # Shape: (num_users, num_poverty_tags)
    food_poverty = food_tags[:, :, -num_poverty_tags:]  # Shape: (num_users, k, num_poverty_tags)
    
    # Expand dimensions for broadcasting
    user_poverty_expanded = user_poverty.unsqueeze(1)  # Shape: (num_users, 1, num_poverty_tags)
    
    # Calculate if there's at least one common poverty tag (intersection > 0)
    common_poverty = torch.logical_and(user_poverty_expanded, food_poverty).sum(dim=2) > 0  # Shape: (num_users, k)
    
    # Calculate the proportion of affordable foods for each user
    poverty_match_ratio = common_poverty.float().mean(dim=1)  # Shape: (num_users)
    
    # Calculate the average poverty affordability across all users
    poverty_affordability = poverty_match_ratio.mean().item()
    
    return poverty_affordability
...
def get_metrics(model, user_tags, food_tags, edge_index, exclude_edge_indices, k, 
                ...
                num_nutrition_tags=18, num_poverty_tags=3):
    """    
    Computes the evaluation metrics: recall, precision, ndcg, H-score, PA @ k

    Args:
        model (LighGCN): lightgcn model
        user_tags: user tags tensor
        food_tags: food tags tensor
        edge_index (torch.Tensor): 2 by N list of edges for split to evaluate
        exclude_edge_indices ([type]): 2 by N list of edges for split to discount from evaluation
        k (int): determines the top k items to compute metrics on
        users_emb_final: final user embeddings
        items_emb_final: final item embeddings
        num_nutrition_tags: number of nutrition-related tags (default 18)
        num_poverty_tags: number of poverty-related tags (default 3)

    Returns:
        tuple: recall @ k, precision @ k, ndcg @ k, health_score, poverty_affordability, 
    ...
    health_score = calculate_health_score(users, top_K_items, user_tags, food_tags, 
                                         num_nutrition_tags=num_nutrition_tags)
    
    poverty_affordability = calculate_poverty_affordability(users, top_K_items, user_tags, food_tags,
                                                           num_poverty_tags=num_poverty_tags)
    ...
    return recall, precision, ndcg, health_score, poverty_affordability, avg_health_tags_ratio, percentage_recommended_foods
...
def eval(model, feature_dict, user_tags, food_tags, edge_index, pos_edge_index, neg_edge_index, 
    ...
    Returns:
        tuple: bpr loss, recall @ k, precision @ k, ndcg @ k, health_score, poverty_affordability, 
               avg_health_tags_ratio, percentage_recommended_foods
    """
    # forward pass
    ...
    recall, precision, ndcg, health_score, poverty_affordability, avg_health_tags_ratio, percentage_recommended_foods = \
        get_metrics(model, user_tags, food_tags, edge_index, exclude_edge_indices, k, users_emb_final, items_emb_final,
                   num_nutrition_tags=num_nutrition_tags, num_poverty_tags=num_poverty_tags)

    return loss, recall, precision, ndcg, health_score, poverty_affordability, avg_health_tags_ratio, percentage_recommended_foods
