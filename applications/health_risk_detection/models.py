...
class mod_RGCN(torch.nn.Module):
    def __init__(self, num_relations, node_feature_dims, num_classes, common_dim=128, hidden_dim=128, dropout=0.6):
        ...
        self.init_health = nn.Linear(node_feature_dims['health_condition'], common_dim)
        self.init_nutrition = nn.Linear(node_feature_dims['nutrition_tag'], common_dim)
        self.init_price = nn.Linear(node_feature_dims['price_tag'], common_dim)
        self.init_poverty = nn.Linear(node_feature_dims['poverty_condition'], common_dim)
    ...
    def forward(self, x_dict, edge_index, edge_type):
        ...
        x_health = self.init_health(x_dict['health_condition'])
        x_nutrition = self.init_nutrition(x_dict['nutrition_tag'])
        x_price = self.init_price(x_dict['price_tag'])
        x_poverty = self.init_poverty(x_dict['poverty_condition'])

        x_all = torch.cat([
            x_user, x_food, x_ingredient, x_category, x_habit,
            x_health, x_nutrition, x_price, x_poverty
        ], dim=0)
...
class RGCN(torch.nn.Module):
    def __init__(self, num_relations, node_feature_dims, num_classes,
                 common_dim=128, hidden_dim=128, dropout=0.6):
        ...
        self.init_category = nn.Linear(node_feature_dims['category'], common_dim)
        self.init_habit = nn.Linear(node_feature_dims['habit'], common_dim)
        self.init_health_condition = nn.Linear(node_feature_dims['health_condition'], common_dim)
        self.init_nutrition_tag = nn.Linear(node_feature_dims['nutrition_tag'], common_dim)
        self.init_price_tag = nn.Linear(node_feature_dims['price_tag'], common_dim)
        self.init_poverty_condition = nn.Linear(node_feature_dims['poverty_condition'], common_dim)
    ...
    def forward(self, x_dict, edge_index, edge_type):
        ...
        x_category = self.init_category(x_dict['category'])
        x_habit = self.init_habit(x_dict['habit'])
        x_health_condition = self.init_health_condition(x_dict['health_condition'])
        x_nutrition_tag = self.init_nutrition_tag(x_dict['nutrition_tag'])
        x_price_tag = self.init_price_tag(x_dict['price_tag'])
        x_poverty_condition = self.init_poverty_condition(x_dict['poverty_condition'])

        x_all = torch.cat([
            x_user, x_food, x_ingredient, x_category, x_habit,
            x_health_condition, x_nutrition_tag, x_price_tag, x_poverty_condition
        ], dim=0)
